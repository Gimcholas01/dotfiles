...
" dont select new line when using $
vnoremap $ g_

" buffer delete, dont close window
map <C-d> :bn<CR>:bd#<CR>

" make mark jump to column also
nnoremap ' `
...
let g:airline#extensions#branch#format = 2  " Simplify git branch name
let g:airline_section_a = '%{airline#util#wrap(airline#parts#mode(),0)}'  " Show current mode
let g:airline_section_b = '%{airline#extensions#branch#head()}'           " Git branch only
let g:airline_section_c = '%F'                                            " Full file path
let g:airline_section_x = ''                                               " Disable filetype/encoding
let g:airline_section_y = ''                                        " File format (e.g., UNIX/DOS)
